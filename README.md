...
- **Грамматика авроры и кнопки** (21.09, вечер): aurora3d.exe рисует ВСЮ аврору (ffmpeg только накладывает слой):
  ...
  `W:\tools\aurora3d\toggles.txt` и легенда в кадре. `push_watchdog.ps1` перезапускает ffmpeg, если aurora3d стартовал
  позже него (иначе слой застывает на последнем кадре). CPU: ffmpeg 16,7 %, aurora3d 1,3 %.
  Подписи значений и устройств — **лучами** в плоскости кромки, от контура наружу, в полосе своих данных; с 22.09 в обоих
  режимах (и «Наизнанку», и обычная труба), плоских 2D-подписей больше нет. Пресеты можно нажимать и без Frigate —
  MQTT `frigate/xps/ptz` ← `preset_<имя строчными>` (например `preset_наизнанку`); журнал нажатий — «кнопка:» в aurora3d.log.
  Сборка без build.ps1: `W:\Windows\Microsoft.NET\Framework64\v4.0.30319\csc.exe /optimize /unsafe /target:winexe
  /r:System.Drawing.dll /r:System.Web.Extensions.dll /r:System.Core.dll` (csc на диске W:, не C:).
  **Писатель канала (24.09.2026):** слой отдаётся в `\\.\pipe\aurora3d` отдельным потоком ровно 25 к/с последним готовым
  кадром — если рисование не успевает (XPS под нагрузкой), кадр повторяется. Раньше запись шла из цикла рисования, и при
  14–18 к/с ffmpeg камеры отставал от реального времени и копил кадры в памяти (11 ГБ за 50 минут).
- **Аврора 25 к/с** (21.09, поздно): рендерер 25 к/с (кэш подписей картинками, таблица «распремножения»), слой
  накладывается ПОСЛЕ fps=25. Грабли: (1) `-use_wallclock_as_timestamps` на входе канала — метка ставится при чтении,
  а читает ffmpeg по нужде наложения: темп залипал на ~3 к/с (это и было «обновляется раз в секунду»), нужны ровные
  метки `-framerate 25`; (2) общие настенные часы для камеры и PNG ломают граф — image2 флаг игнорирует, pts ≈ −1,8·10⁹,
  ни одного кадра. Цена: ffmpeg 23 %, aurora3d 8 %, NVIDIA 3D 2,3 %, копирование 4,9 %; CPU XPS ~79 °C.
  Потом: срез водопада/тоннеля на каждый кадр анализа (10/с, 200 срезов = 20 с), вершины среза строятся один раз,
  вглубь — glTranslatef, затухание — туман OpenGL: рендерер 25,0 к/с, aurora3d 4,6 %, ffmpeg 17,7 %, NVIDIA 3D 3,7 %.
- **Тоннель грамматик** (21.09, ночь): веер убран; все 10 грамматик — 3D, делят круг поровну (десятигранный тоннель,
  грань i повёрнута на i·36° вокруг оси взгляда), у кромки «сейчас», вглубь к центру — 20 с прошлого, кольца времени через
  5 с. Второй проход на GPU (шейдер GLSL 1.10: альфа = max(r,g,b), «распремножение», переворот) вместо CPU-цикла по
  518 тыс. пикселей; угасание срезов — glBlendColor. Камера — MJPEG 1080p 25 к/с, анализ авроры 25 к/с (xps_sub 25 к/с,
  400 кбит/с), запись xps_cam — 10 к/с. Грабля WinRM: «maximum number of concurrent operations» у учётки MCP после серии
  таймаутов — лечится перезапуском службы WinRM разовой задачей SYSTEM (процесс, запущенный из WinRM-сессии, умирает с ней).
- **Аврора на GPU целиком** (21.09, ночь): слой сразу 1920x1080; подписи — текстуры (GDI+ растрирует строку один раз,
  дальше это текстура на NVIDIA); итог (свечение + подписи → прямая альфа → yuva420p: Y, U/V на ½, A) — 4 прохода
  шейдера, ffmpeg берёт `-pix_fmt yuva420p -s 1920x1080` без перевода цвета и растяжения. Тоннель наезжает (новое — из
  глубины), медленно крутится (оборот 90 с), названия грамматик — снаружи у кромки своей грани (на верхних гранях
  повёрнуты на 180°). Бегущий квадратик ffmpeg внизу в режиме авроры заменён часами по периметру кадра (круг 60 с,
  риски 1 с, подписи 5 с). Кадр рендерера 40 мс: сцена 6, подписи 1, итог+считывание 13, канал 8. NVIDIA 3D ~16 %.
- **Bloom и «Наизнанку»** (21.09, ночь): ореол свечения на GPU (¼ разрешения, гаусс 9 выборок, 2 прохода по двум
  осям, прибавляется в итоговом шейдере; подписи без ореола). Пресет «Наизнанку» (по умолчанию вкл): труба выворачивается
  в столб — колонна TR 0,55, данные граней растут наружу; сделано отражением грани в матрице (WallBegin), код грамматик
  общий; состояние — строка «Наизнанку=» в toggles.txt. Новый пресет Frigate видит только после своего перезапуска.
- **Сторож TextInputHost** (`tools/tih_watchdog.ps1` → `W:\tools\`, задача `TextInputHostWatchdog` под rdpuser,
  ...
  изменения буфера обмена) и перезапускает процесс (Windows поднимает его сама). Причина пока не установлена —
  подозрения: история буфера обмена (Win+V) со снимками экрана от скриптов A56, переключение языка AutoHotkey.
...
## Сторож процессов (2026-09-24)

`tools/proc_guard.ps1` → `W:\tools\proc_guard\`, задача **ProcGuard** (SYSTEM, раз в минуту и при старте). Вместо
перезапусков — приоритеты и лимиты: диктовка `dictate.py` — High, ffmpeg камеры — AboveNormal, служба
`DeviceAssociationService` (зацикливается на поиске пропавшего устройства) — Idle + режим эффективности + жёсткий
потолок CPU 3 % через Job Object; страховка — ffmpeg камеры > 3 ГБ → перезапуск WebcamPush. Журнал `proc_guard.log`
пишется только при изменениях. Defender — защищённый процесс, к нему применимы только исключения.
